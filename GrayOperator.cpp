...
void ThresholdTrans(BYTE* pixel, BYTE* tempPixel, UINT width, UINT height, UINT rowlen, UINT channel, int thr)
{
	...
	for (UINT i = 0; i < height; i++)
	{
		...
		{
			BYTE *pSrc = pixel + j * channel + i * rowlen;
			BYTE *pCur = tempPixel + j * channel + i * rowlen;
			*pCur = *(pCur + 1) = *(pCur + 2) = map[*pSrc];
		}
	}
}
...
*		width		原始图像宽度
******************************************************************************/
void GrayEqualize(BYTE* pixel, BYTE* tempPixel, UINT width, UINT height, UINT rowlen, UINT channel)
{
