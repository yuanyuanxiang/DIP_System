...
METHODDEF(void) my_error_exit (j_common_ptr cinfo)
{
	...
	USES_CONVERSION;
	MessageBox(NULL, A2W(buffer), L"JPEG Fatal Error", MB_ICONSTOP);
...

{
	...
	*/
	...
	USES_CONVERSION;
	if ((infile = fopen(W2A(fileName), "rb")) == NULL) {
		sprintf(buf, "JPEG :\nCan't open %s\n", W2A(fileName));
		AfxMessageBox(A2W(buf));
		return NULL;
	}
							...
							int quality)
{
	...
	USES_CONVERSION;
	if ((outfile = fopen(W2A(fileName), "wb")) == NULL) {
		char buf[250];
