...
// CEnhancedHistDlg 对话框

IMPLEMENT_DYNAMIC(CEnhancedHistDlg, CDialog)

CEnhancedHistDlg::CEnhancedHistDlg(CWnd* pParent /*=NULL*/) : CDialog(CEnhancedHistDlg::IDD, pParent)
	, m_nLimitLow(0)
	, m_nLimitUp(255)
	, m_nGray(0)
	, m_dPer(0)
...
CEnhancedHistDlg::~CEnhancedHistDlg()
{
}

void CEnhancedHistDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CEnhancedHistDlg, CDialog)
	ON_EN_CHANGE(IDC_LIMIT_LOWER, &CEnhancedHistDlg::OnEnChangeLimitLower)
	ON_EN_CHANGE(IDC_LIMIT_UP, &CEnhancedHistDlg::OnEnChangeLimitUp)
	ON_WM_PAINT()
	ON_WM_MOUSEMOVE()
	ON_WM_SETCURSOR()
	ON_WM_LBUTTONDOWN()
...
// CEnhancedHistDlg 消息处理程序

/******************************************************************************
*	作用:		对话框初始化时计算各级灰度数量
******************************************************************************/
BOOL CEnhancedHistDlg::OnInitDialog()
{
	...
	int channel = pDoc->m_pDIB->Channel();
	LONG lRowLen = pDoc->m_pDIB->RowLen();
	BYTE* pSrc = pDoc->m_pDIB->FindAddress();
	for (UINT i = 0; i < height; i++)
	{
		for (UINT j = 0; j < width; j++)
		{
			BYTE *pCur = pSrc + j * channel + i * lRowLen;
			m_lCounts[int (*pCur * 0.299 + *(pCur + 1) * 0.587 + *(pCur + 2) * 0.114)]++;
		}
	}
...
*	作用:		灰度下限改变的响应函数
******************************************************************************/
void CEnhancedHistDlg::OnEnChangeLimitLower()
{
	UpdateData(TRUE);
...
*	作用:		灰度上限改变的响应函数
******************************************************************************/
void CEnhancedHistDlg::OnEnChangeLimitUp()
{
	UpdateData(TRUE);
...
*	作用:		鼠标移动时响应托拽动作
******************************************************************************/
void CEnhancedHistDlg::OnMouseMove(UINT nFlags, CPoint point)
{
	CRect rect;
...
*	作用:		托拽时改变鼠标光标
******************************************************************************/
BOOL CEnhancedHistDlg::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	CRect rect;
	CPoint point;
...
******************************************************************************/
void CEnhancedHistDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	CRect rect;
...
*	作用:		释放鼠标的响应函数
******************************************************************************/
void CEnhancedHistDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	m_nIsDraging = DT_NULL;
}
